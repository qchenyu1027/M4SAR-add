...
from ultralytics.nn.modules.common import Iterative_Differential_TransformerFusionBlock, SSF,MultiTransformerBlock
...
__all__ = (
    ...
    "ICAFusion",    # M4-SAR
    "AAttenADD",
    "Iterative_Differential_TransformerFusionBlock",##IRD
    "SSF",##MS2FUSION
    "KanConvFusionBlock",
...
class FastKANConvNDLayerkanfusion(nn.Module):
...
class FastKANConv2DLayer1(FastKANConvNDLayerkanfusion):
...
class ResidualRouter(nn.Module):
    def __init__(self, input_dim):
        super().__init__()
        # 一个简单的 SE-Like 门控，输出 1 个通道的权重 (0~1 之间)
        ...
        # --- 关键优化：初始化为 0 ---
        # 这样训练开始时，gate 输出全是 0.5 (Sigmoid(0)) 附近
        # 我们手动把最后一层 bias 设为负数，让初始 gate 接近 0
        nn.init.constant_(self.gate[-2].bias, -3.0)
        nn.init.normal_(self.gate[-2].weight, std=0.001)

    def forward(self, x):
        return self.gate(x)  # [B, 1, 1, 1]


class MultiScaleSymbolicFeature(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_dim=64,
                 num_scales=3, num_funcs=4):
        super().__init__()
        ...
        self.fusion_conv = nn.Conv2d(out_channels * num_scales * num_funcs, out_channels, 1)
        self.freqs = nn.Parameter(torch.randn(num_scales, out_channels))
        # --- ③ 元学习高斯基参数 ---
        self.centers = nn.Parameter(torch.randn(num_scales, out_channels))
        self.scales = nn.Parameter(torch.ones(num_scales, out_channels))

        # --- ④ 自适应门控权重 ---
        self.gate = nn.Linear(out_channels, num_scales * num_funcs)
        self.alpha = nn.Parameter(torch.randn(num_scales, num_funcs))

        # --- ⑤ 融合层 ---
        self.proj = nn.Conv2d(out_channels, hidden_dim, kernel_size=1)
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        feats = []
        gate = self.gate(x.mean(dim=[2, 3]))  # [B, num_scales * num_funcs]
        gate = gate.view(B, self.num_scales, self.num_funcs)

        for i in range(self.num_scales):
            y = self.scale_convs[i](x)  # [B, C, H, W]

            # 元学习高斯调制
            diff = y - self.centers[i].view(1, -1, 1, 1)
            gauss = torch.exp(-0.5 * (diff / (self.scales[i].view(1, -1, 1, 1) + 1e-6)) ** 2)
            for j, func in enumerate(self.funcs):
                   ##ablation
                    gate_weight = (torch.sigmoid(self.alpha[i][j]) * gate[:, i, j].view(-1, 1, 1, 1))
                    # feats.append(gate_weight * func(gauss))
                    if func is torch.sin or func is torch.cos:
                        freq_term = gauss * self.freqs[i].view(1, -1, 1, 1)
                        feat = func(freq_term)
                    else:
                        feat = func(gauss)
                    feats.append(gate_weight * feat)

            # 融合所有尺度和函数的特征
            # stack [num_scales * num_funcs, B, C, H, W] -> mean -> [B, C, H, W]
        if len(feats) > 0:
                # fused = torch.stack(feats, dim=0).mean(dim=0)
                # out = self.out_conv(self.act(self.proj(fused)))
...
class FastKANConvNDLayer(nn.Module):
    def __init__(self, conv_class, norm_class, input_dim, output_dim, kernel_size,
                 taylor_order=3, groups=1, padding=0, stride=1, dilation=1,
                 ndim: int = 2, grid_size=8, base_activation=nn.SiLU, grid_range=[-2, 2], dropout=0.0):
        super(FastKANConvNDLayer, self).__init__()
        self.inputdim = input_dim
        self.outdim = output_dim
        ...
        self.layer_norm = nn.ModuleList([
            norm_class(input_dim // groups) for _ in range(groups)
        ])

        # 2. 主路：Base Conv 
        self.base_conv = nn.ModuleList([
            conv_class(input_dim // groups, output_dim // groups, kernel_size, stride, padding)
            for _ in range(groups)
        ])
        self.cross_align_rgb2ir = nn.Conv2d(output_dim // groups, output_dim // groups, 1)
        self.cross_align_ir2rgb = nn.Conv2d(output_dim // groups, output_dim // groups, 1)

        # 3. 辅路：Symbolic KAN 
        self.symbolic_feature_rgb = MultiScaleSymbolicFeature(input_dim // groups, output_dim // groups)
        self.symbolic_feature_ir = MultiScaleSymbolicFeature(input_dim // groups, output_dim // groups)

        # 4. 门控：Residual Router
        self.routers = nn.ModuleList([
            ResidualRouter(input_dim // groups)
            for _ in range(groups)
        ])
    ...
    def forward_fast_kan(self, rgb_x, ir_x, group_index):
        ...
        rgb_base, ir_base = None, None
        # 2. 计算主路 (Base)
        rgb_base = self.base_conv[group_index](rgb_norm)
        ir_base = self.base_conv[group_index](ir_norm)

        # 3. 计算辅路 (KAN)
        rgb_kan = self.symbolic_feature_rgb(rgb_norm)
        ir_kan = self.symbolic_feature_ir(ir_norm)

        # 4. 计算门控权重 (0~1)
        rgb_gate = self.routers[group_index](rgb_norm)
        ir_gate = self.routers[group_index](ir_norm)

        aligned_ir_kan = self.cross_align_ir2rgb(ir_kan)
        aligned_rgb_kan = self.cross_align_rgb2ir(rgb_kan)

        # 5. 残差融合: Base + Gate * KAN
        rgb_out = rgb_base + rgb_gate * aligned_ir_kan
        ir_out = ir_base + ir_gate * aligned_rgb_kan
        # rgb_out = rgb_base + rgb_gate * rgb_kan
        # ir_out = ir_base + ir_gate * ir_kan
    ...
    def forward(self, x):
        ...
        rgb_outputs, ir_outputs = [], []
        for i, (r, t) in enumerate(zip(split_rgb, split_ir)):
            rgb_out, ir_out = self.forward_fast_kan(r, t, i)
            rgb_outputs.append(rgb_out)
            ir_outputs.append(ir_out)
